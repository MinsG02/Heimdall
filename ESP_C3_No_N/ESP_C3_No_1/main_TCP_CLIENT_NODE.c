 ...
 * 기능: TDOA 노드 (TCP Client)
 * 개선: Trigger 후 2초간 쿨다운 (중복 전송 방지 & 버퍼 비우기)
 */
...
#include <sys/time.h>
#include <time.h>
#include <errno.h>             // [필수] 에러 코드용 헤더 추가
#include <arpa/inet.h>         // [필수] IP 주소 변환용 헤더 추가
...
// ================= [★ 사용자 설정: 보드마다 바꿔주세요] =================
#define MY_NODE_ID      "NODE_JEW"       // NODE_1, NODE_2, NODE_3 ... NODE_5
...
#define QUIET_ROOM_DB       25.0 // 조용한 상태 기준값
#define TRIGGER_MARGIN      5.0  // 30dB 이상일 때 전송
// =====================================================================
...
static const char *TAG = "HEIMDALL_TCP";
...
int connect_tcp_server() {
    ...
    ESP_LOGI(TAG, "Connecting to %s:%d...", RPI_IP_ADDR, TCP_PORT);
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, ">>> System Running. Waiting for BANG! (>%.1fdB) <<<", g_trigger_threshold);

    while (1) {
        // TCP 재연결 로직
        if (sock < 0) {
            sock = connect_tcp_server();
            if (sock < 0) {
        ...
        if (i2s_channel_read(rx_handle, samples, 512 * sizeof(int32_t), &bytes_read, 1000) == ESP_OK) {
            ...
            if (calibrated_db > g_trigger_threshold) {
                ...
                // 1. 패킷 생성 (형변환 추가로 에러 방지)
                int len = snprintf(tx_buffer, sizeof(tx_buffer), "%s,%lld,%.2f,%.1f", 
                                 MY_NODE_ID, (long long)time_us, calibrated_db, limit);
                ...
                if (err < 0) {
                    ...
                    // ========================================================
                    ESP_LOGI(TAG, ">>> Cooldown: 2.0 sec (Ignoring echoes) <<<");
                    vTaskDelay(pdMS_TO_TICKS(2000)); // 2초간 멈춤
                    
                    // I2S 버퍼에 남아있는 잔향(Ring) 데이터를 읽어서 버림
                    size_t dump_bytes;
                    // 약 2~3번 읽어내면 버퍼가 비워짐
